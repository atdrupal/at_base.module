...
class Cache {
   ...
   */
  public function get() {
    if (!$this->reset && $cache = atcg('wrapper.cache')->get($this->id, $this->bin)) {
  ...
  protected function write($data) {
    if (FALSE !== atcg('wrapper.cache')->set($this->id, $data, $this->bin, strtotime($this->ttl))) {
      if (!empty($this->tags)) {
        $this->removeAllTags();
  ...
  public function addTag($tag) {
    return atcg('wrapper.db')->insert('at_base_cache_tag')
      ->fields(array(
          'bin' => $this->bin,
          'cid' => $this->id,
          'tag' => $tag,
  ...
  public function removeAllTags() {
    return atcg('wrapper.db')->delete('at_base_cache_tag')
